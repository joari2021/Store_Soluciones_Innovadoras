...
class VentasControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'show masks restricted and caution service names for non admin users' do
    ...
    assert_response :success
    assert_includes response.body, "Servicio #{caution_system.name}"
    assert_includes response.body, "Servicio #{restricted_system.name}"
  ...
  def create_auto_cost_service!(service_price_usd:, cost_units:)
    ...
    structure = service.service_expense_structures.create!(description: 'Costo base')
