class VentasController < ApplicationController
  ...
  def index
    ...
    @services_payload = @services.map do |service|
      ...
      unit_cost_usd = if service.auto_cost_stock_discount?
                        service.total_expense_usd(tasa_dolar: tasa_dolar, unidad_vi: unidad_vi)
                      end
  ...
  def sale_item_masked_names_for_view(venta)
    ...
    service_items.each_with_object({}) do |item, masked_names|
      ...
      system_name = item.variation_name.to_s.strip.presence ||
                    candidates.filter_map { |service| service.system_service&.name.to_s.strip.presence }.first
      masked_names[item.id] = system_name.present? ? "Servicio #{system_name}" : 'Servicio'
    end
  end
  ...
  def collect_service_product_consumptions!(service:, multiplier:, grouped:, visited_service_ids:,
    ...
    service.service_expense_structures.includes(
      {
        service_product_expenses: [:product_variation, { producto: :product_variations }]
      },
      { service_nested_expenses: :nested_service }
  ...
  def build_service_cost_obligations(service_item_rows:, tasa_dolar:)
    ...
    grouped_rows.each_value do |row|
      ...
      unit_cost_usd = service.total_expense_usd(tasa_dolar: tasa_dolar, unidad_vi: unidad_vi).to_d.round(2)
      unless unit_cost_usd.positive?
        return [[],
                "El servicio #{service.description} tiene activo el descuento automatico, pero no tiene costos configurados."]
      end
